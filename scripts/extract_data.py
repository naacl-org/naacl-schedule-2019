...
The actual files needed to generate the schedules are:

- {N19,W19,S19}.xml, the XML files containing the author metadata,
  abstract, and anthology links for all papers being presented at
  the conference, workshops, and other co-located events. These are
  extracted by this script.

- The `order` file for the main conference. Note that this file
  is not actually generated by this script. This file needs to be
  created by combining the `order` files for for the various
  components of the main conference (main papers, industry track
  papers, demos, and SRW papers). If the program chairs or
  pub chairs do not provide such a combined order file for the main
  conference, then the website/app chair will need to do the combination
  themselves. For NAACL 2019, I opted to do the combination manually due
  to time constraints but it should not be too difficult to do the
  combination automatically too barring any mismatches between the
  order files.

- `order` files for the workshops and co-located events. These are
   extracted by this script.

- `id_map.txt` for the main conference. This file contains the mappings
  between the anthology IDs and the actual START IDs of the papers/posters.
  The order files use START IDs but do not contain any author metadata.
  The author metadata needs to be extracted from the XML files but those
  files use the anthology IDs. Therefore, a mapping is necessary. Just like
  the `order` file for the main conference, this file was also generated
  manually by combining the `id_map.txt` files for the main conference
  components. This file is not extracted by this script.

- `id_map.txt` files for the various workshops and co-located events.
  These files are extracted by this script.
